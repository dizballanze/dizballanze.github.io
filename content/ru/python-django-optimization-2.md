...
В этой части часто будет использоваться логирование SQL запросов и DDT, настройка которых описана в первом посте.
...
Также предположим, что мы экспортировали данные из старого приложения в огромные JSON файлы. Файл с авторами имеет
следующий вид:
...
Запустив команду, описанным выше способом, мы увидим, что был выполнен один огромный запрос к БД!
...
команда использует постоянный объем памяти. Вот графики для ~200Mb статей (без `iterator` и с ним соответственно):
...
Теперь размер выборки ограничивается в запросе: `...LIMIT 20`. Также показано, что отправка запроса
